...
TEST_F(ProcessInfoNodeStartupTest, SelfTerminating_TerminatedReadyCondition_ReapedWhileWaitingForkRunning_ReturnsSuccess)
{
    RecordProperty(
        "Description",
        "A self-terminating reporting process whose ready condition is Terminated and that is reaped while the startup "
        "thread is still waiting for kRunning reports success instead of losing its completion.");

    auto node = createProcessInfoNode(
        configuration::ApplicationType::Reporting, 0U, true, configuration::ProcessState::Terminated);
    expectSuccessfulProcessLaunch();
    // Simulate the OS handler reaping the process while the startup thread is blocked in waitForkRunning,
    // i.e. before it could set kRunning. This is the race behind the flaky sandbox_options test (#503).
