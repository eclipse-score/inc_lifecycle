...
constexpr NanoSecondType k_maxRemainderNanoSec{std::numeric_limits<NanoSecondType>::max() -
                                               (k_maxSeconds * TimeConversion::k_nanoSecInSec)};
...
TEST_F(TimeConversionTest, ConvertToNanoSec_ZeroTimespec_ReturnsZero)
{
    RecordProperty("Description",
                   "This test verifies that a zero-valued timespec is converted to zero nanoseconds.");
...
TEST_F(TimeConversionTest, ConvertToNanoSec_SecondsOnly_ReturnsSecondsInNanoSec)
{
    ...
    ASSERT_EQ(TimeConversion::convertToNanoSec(ts),
              2U * static_cast<NanoSecondType>(TimeConversion::k_nanoSecInSec));
}

TEST_F(TimeConversionTest, ConvertToNanoSec_NanoSecondsOnly_ReturnsNanoSeconds)
{
    RecordProperty("Description",
                   "This test verifies that a timespec containing only a nanosecond part is returned unchanged.");
...
TEST_F(TimeConversionTest, ConvertToNanoSec_SecondsAndNanoSeconds_ReturnsSum)
{
    ...
    ASSERT_EQ(TimeConversion::convertToNanoSec(ts),
              static_cast<NanoSecondType>(TimeConversion::k_nanoSecInSec) + 500U);
}
...
TEST_F(TimeConversionTest, ConvertMilliSecToNanoSec_WholeMillis_ReturnsNanoSeconds)
{
    RecordProperty("Description",
                   ...
                   "nanoseconds.");
    ASSERT_EQ(TimeConversion::convertMilliSecToNanoSec(1.0),
...
TEST_F(TimeConversionTest, ConvertMilliSecToNanoSec_Overflow_ReturnsMax)
{
    ...
    ASSERT_EQ(TimeConversion::convertMilliSecToNanoSec(std::numeric_limits<double>::max()),
...
TEST_F(TimeConversionTest, ConvertNanoSecToMilliSec_WholeMilli_ReturnsMilliSeconds)
{
    RecordProperty("Description",
                   ...
                   "milliseconds.");
    ASSERT_DOUBLE_EQ(TimeConversion::convertNanoSecToMilliSec(
                         static_cast<NanoSecondType>(TimeConversion::k_nanoSecInMilliSec)),
                     1.0);
}
