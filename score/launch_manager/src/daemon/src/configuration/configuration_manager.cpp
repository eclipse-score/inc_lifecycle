...
constexpr const char* kEnvVarName = "ECUCFG_ENV_VAR_ROOTFOLDER";                         ///< Environment variable name
constexpr const char* kEnvVarDefaultValue = "/opt/internal/launch_manager/etc/ecu-cfg";  ///< Environment variable value

const uint32_t ConfigurationManager::kDefaultProcessExecutionError = 1U;
uint64_t ConfigurationManager::kDefaultProcessorAffinityMask()
{
    const auto n = osal::getNumCores();
    return (n >= 64U) ? UINT64_MAX : ((1ULL << n) - 1ULL);
}
                                                                                ...
                                                                                const uint32_t index) const
{
    ...
    {
        LM_LOG_DEBUG() << "Unable to retrieve process configuration for process group" << pg_name << "with index"
                       << index;
    }
    ...
    for (auto& process_group : process_groups_)
    {
        for (auto& process : process_group.processes_)
        {
            for (size_t i = 0U; i < score::lcm::internal::kArgvArraySize && process.startup_config_.argv_[i] != nullptr;
                 ++i)
            {
                // RULECHECKER_comment(1, 1, check_pointer_qualifier_cast_const, "Remove const for standard library with
                // char type arguments.", true);
...
bool ConfigurationManager::parseMachineConfigurations(const ModeGroup* node, const IdentifierHash& cluster)
{
    ...
    if (node)
    {
        ...
        LM_LOG_DEBUG() << "FlatBufferParser::getModeGroupPgName:"
                       << std::string_view{getStringFromFlatBuffer(node->identifier())}
...
static void setSchedulingParameters(const Process& node, const ProcessStartupConfig& config, OsProcess& instance)
{
    ...
    if (attribute != nullptr) {
        ...
        {
            LM_LOG_WARN() << "scheduling policy" << std::string_view{attribute->c_str(), attribute->size()}
                          << "is not supported, using default";
        }
    }
    ...
    if (attribute != nullptr)
    {
        instance.startup_config_.cpu_mask_ = std::stoull(attribute->c_str());
    }
}
    ...
    if (env_var_list)
    {
        ...
        for (size_t i = 0U; i < max_env; ++i)
        {
            ...
            if (env_var_node)
            {
                ...
                // coverity[autosar_cpp14_a4_7_1_violation:FALSE] Environment count is checked above - no risk of
                // wraparound.
...
ProcessGroup* ConfigurationManager::getProcessGroupByID(const IdentifierHash& pg_name) const
{
    ...
    {
        auto it = find_if(process_groups_.begin(), process_groups_.end(), [&pg_name](const ProcessGroup& pg) {
            return pg.name_ == pg_name;
        });
...
ProcessGroupState* ConfigurationManager::getProcessGroupStateByID(const ProcessGroupStateID& pg_id) const
{
    ...
    if (pg)
    {
        auto it = find_if(pg->states_.begin(), pg->states_.end(), [&pg_id](const ProcessGroupState& state) {
            return state.name_ == pg_id.pg_state_name_;
        });
                                                                  ...
                                                                  const IdentifierHash& state_name) const
{
    ...
    auto it = find_if(pg.states_.begin(), pg.states_.end(), [&state_name](const ProcessGroupState& state) {
        return state.name_ == state_name;
    });
                                                                 ...
                                                                 const OsProcess& process_instance)
{
    ...
    if (pg != nullptr)
    {
        ...
        if (state != nullptr)
        {
            ...
            auto it = find_if(
                pg->processes_.begin(), pg->processes_.end(), [&process_instance](const OsProcess& inst) -> bool {
