...
void ConfigurationAdapter::deinitialize()
{
    for (auto& process_group : process_groups_)
    {
        ...
        {
            for (size_t i = 0U;
                 i < score::mw::lifecycle::internal::kArgvArraySize && process.startup_config_.argv_[i] != nullptr;
                 ++i)
            {
    ...
    score::mw::lifecycle::internal::osal::OsalConfig& startup) const
{
    ...
    assert(
        props.process_arguments.size() <= score::mw::lifecycle::internal::kMaxArg &&
        "Too many process arguments for argv array");
    size_t max_args =
        std::min(props.process_arguments.size(), static_cast<size_t>(score::mw::lifecycle::internal::kMaxArg));
...
std::vector<ProcessGroupState> ConfigurationAdapter::buildProcessGroupStates(const Config& config) const
{
    ...
    const bool has_off = std::any_of(states.cbegin(), states.cend(), [&](const ProcessGroupState& s) {
        return s.name_ == off_name;
    });
    if (!has_off)
    {
...
bool ConfigurationAdapter::buildFromConfig(const Config& config)
{
    ...
    LM_LOG_DEBUG() << "ConfigurationAdapter: Built configuration with " << process_groups_[0].processes_.size()
                   << " processes and " << process_groups_[0].states_.size() << " states";
...
std::optional<const score::mw::lifecycle::internal::ProcessGroupStateID*> ConfigurationAdapter::getMainPGStartupState()
    const
{
    ...
    const uint32_t index) const
{
    ...
    LM_LOG_DEBUG() << "Unable to retrieve process configuration for process group" << pg_name << "with index" << index;
    return std::nullopt;
}
    ...
    const uint32_t index) const
{
    ...
    LM_LOG_DEBUG() << "Unable to retrieve process dependencies for process group" << pg_name << "with index" << index;
    return std::nullopt;
}
